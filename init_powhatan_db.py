#!/usr/bin/env python3
"""Build / migrate powhatan_dictionary.db from licensed CSVs."""
...
DB_PATH = ROOT / "powhatan_dictionary.db"
SEED = ROOT / "powhatan_seed.csv"

SCHEMA = """
CREATE TABLE IF NOT EXISTS dictionary (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    powhatan_word TEXT NOT NULL,
    english_translation TEXT NOT NULL,
    word_class TEXT,
    source TEXT,
    license TEXT,
    orthography TEXT DEFAULT 'Historical',
    notes TEXT,
    audio_url TEXT
);
CREATE INDEX IF NOT EXISTS idx_powhatan_word ON dictionary(powhatan_word);
...
def load_csv(conn, path: Path) -> int:
    ...
    with path.open(encoding="utf-8-sig") as f:
        ...
        required = {"powhatan_word", "english_translation", "source", "license"}
        missing = required - set(reader.fieldnames or [])
        if missing:
            raise SystemExit(f"{path.name} missing columns: {missing}")
        for row in reader:
            word = (row.get("powhatan_word") or "").strip()
            ...
            conn.execute(
                ...
                (powhatan_word, english_translation, word_class, source, license,
                ...
                (
                    ...
                    row["source"].strip(),
                    row["license"].strip(),
                    (row.get("orthography") or "Historical").strip(),
